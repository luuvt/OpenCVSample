...
class ViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    var detector = BlobDetectorBridge()
	...
	override func viewDidLoad() {
        ...
        self.session.sessionPreset = AVCaptureSession.Preset.hd1280x720 // not work in iOS simulator
		...
		do {
			...
			self.device.activeVideoMinFrameDuration = CMTimeMake(value: 1, timescale: 25) // 20 fps
	...
	func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
		...
		// let resultImage = LaneDetectorBridge.cvtColorBGR2GRAY(capturedImage)
        let resultImage = detector.processDetect(capturedImage)
        // let imageWithLaneOverlay = LaneDetectorBridge.detectLane(in: capturedImage)
		// Show the result.
